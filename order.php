...
class CtrApiClient {
  ...
  * @var string
  */
    protected $apiKey = '62b5d1f5984a1704d773964c9cfcf49f';
    protected $lead = '8149';
...
$namet = trim($_POST["fio"]);
$phonet = trim($_POST["phone"]); 

 $messagetele = "Имя: $namet  \n Телефон: $phonet \n $emailt \n $sitename   ";
 ...
 if( $curl = curl_init() ) {
    curl_setopt($curl, CURLOPT_URL, 'https://api.telegram.org/bot1374453920:AAEPOTIGdpIpw9ijBA6Lzc9RvsWeNqyi8bk/sendMessage');
    curl_setopt($curl, CURLOPT_RETURNTRANSFER,true);
    curl_setopt($curl, CURLOPT_POST, true);
    curl_setopt($curl, CURLOPT_POSTFIELDS, $datatele);
    $out = curl_exec($curl);
    curl_close($curl);
}
